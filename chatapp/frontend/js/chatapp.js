...
`;
    ...
    function connectWebSocket() {
        const socket = new SockJS(`${window.BASE_URL}/ws`);
    ...
    if (logout) {
        logout.onclick = async function () {
            try {
                await fetch(`${window.BASE_URL}/api/users/logout`, {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${token}`,
    ...
    async function loadOnlineUsers() {
        try {
            const res = await fetch(`${window.BASE_URL}/api/users/online`, {
    ...
    async function loadConversation(user) {
        try {
            const res = await fetch(`${window.BASE_URL}/api/messages/conversation/${currentUsername}/${user}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            if (res.ok) {
    ...
    privateUsersList.addEventListener('click', (e) => {
        ...
        chatHeader.innerHTML = `
           <div class="chat-header-user">
            <button style="margin-right: 0;" class="menu-toggle" id="menu-toggle">☰</button>
            <span>Chat with </span>
        ...
        const privateMenu = document.getElementById('menu-toggle');
        privateMenu.addEventListener("click", () => {
    ...
    document.getElementById("create-room-form").addEventListener("submit", async function (e) {
        ...
        try {
            const response = await fetch(`${window.BASE_URL}/api/rooms/create`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
    ...
    // In your room rendering function, ensure all rooms are displayed
    ...
    function joinRoomUI(room) {
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
            ...
            <button onclick="leaveRoom()" class="leave-btn"><img src="https://myimgs.org/storage/images/8002/leave-room.png" alt="Leave" style="cursor: pointer; border : 1px solid cyan; padding : 2px; background-color : cyan;"></button>
        </div>
        <div class="room-menu">
            <button onclick="toggleDropdown()" id="roomMenuBtn">＋</button>
            <div id="roomDropdown">
    ...
    `;
        ...
        const roomMenu = document.getElementById('menu-toggle');
    ...
    document.getElementById("chat-form").addEventListener("submit", async (e) => {
        ...
        try {
            const response = await fetch(`${window.BASE_URL}/rooms/${window.activeRoom.id}/messages`, {
                method: "POST",
    ...
    async function loadRooms(username) {
        try {
            ...
            const response = await fetch(`${window.BASE_URL}/api/rooms/my?user=${encodeURIComponent(username)}`, {
                headers: {
                    "Authorization": `Bearer ${token}`,
                    "Content-Type": "application/json"
                }
    ...
    async function loadRoomMessages(roomId) {
        try {
            const res = await fetch(`${window.BASE_URL}/api/messages/room/${roomId}`, {
                headers: { "Authorization": `Bearer ${token}` }
            });
    ...
    window.joinRoom = async function joinRoom(roomId, username) {
        let activeRoomId;
        try {
            const response = await fetch(`${window.BASE_URL}/api/rooms/${roomId}/join?username=${username}`, {
                method: "POST",
                headers: {
                    "Authorization": "Bearer " + localStorage.getItem("token"),
                    "Content-Type": "application/json"
    ...
    window.leaveRoom = function leaveRoom() {
        if (confirm("Are you sure you want to leave this room?")) {
            if (currentChat && currentChat.type === "room") {
                ...
                console.log("Leaving room:", currentChat.roomId, "as user:", currentUsername);

                fetch(`${window.BASE_URL}/api/rooms/${currentChat.roomId}/leave`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": "Bearer " + localStorage.getItem("token")
                        // Uncomment if your backend requires JWT/session
                        // "Authorization": "Bearer " + localStorage.getItem("token")
                    },
                    body: JSON.stringify({ username: currentUsername }),
                    // credentials: "include" // use if Spring Security with session
                })
                    .then(res => {
                        if (!res.ok) {
                    ...
                    .then(message => {
                        alert(message || `You left the room: ${currentChat.roomId}`);
                        // 3. Reset UI
                        currentChat = null;
                        window.location.reload();
                    })
    ...
    window.showRoomInfo = async function (roomId) {
        try {
            const res = await fetch(`${window.BASE_URL}/api/rooms/${roomId}`, {
